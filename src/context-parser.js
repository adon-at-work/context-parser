...
function Canonicalize(state, i, endsWithEOF) {
    ...
    var charCode, reCanonicalizeNeeded = true,
    ...
    // prefix RAWTEXT (e.g., <[/]style>) end tag with <!--<%%>--> 
        ...
        // and that the end tag name are equal to the start tag name
        // therefore next few chars must equal to the start tag, and
        // it must be appended with either a GREATER, SPACE, or SOLIDUS char
        if (this.input.slice(i - 6, i - 1) !== '%>-->' &&
            this.input.slice(i + 1, i + 1 + this.tags[0].length).toLowerCase() === this.tags[0] &&
            ((charCode = this.input.charCodeAt(i + 1 + this.tags[0].length)) === 62 || charCode === 47 || charCode === 32)) {
